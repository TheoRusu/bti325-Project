...
module.exports.initialize = function() {
	return new Promise((resolve, reject) => {
		let db = mongoose.createConnection(
			'mongodb+srv://dbUser:WebSkills123@senecaweb.j6b02.mongodb.net/a6?retryWrites=true&w=majority'
		);
		db.on('error', (err) => {
			reject(err);
		});
...
module.exports.registerUser = function(userData) {
	return new Promise((resolve, reject) => {
		if (!userData.password || !userData.password.trim() || !userData.password2 || !userData.password2.trim()) {
		...
		User.find({ userName: userData.userName })
			...
			.then((foundUser) => {
				if (foundUser.length < 1) {
					...
					bcrypt.compare(userData.password, foundUser[0].password).then((res) => {
						if (res === true) {
